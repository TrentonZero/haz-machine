...
updateStackHead
  :: MemoryMap -> (MemoryCell -> MemoryCell) -> MemoryMap
updateStackHead state function = 
  ...
  in if firstTuple == Nothing
        then state
                   ...
                   (function (fromJust firstTuple)) : (stack (snd tupleStack))
             in updateStack state result

popFromStack
  :: MemoryMap -> (Maybe MemoryCell,MemoryMap)
popFromStack state = 
...
popFromStackInt
  :: [MemoryCell] -> (Maybe MemoryCell,[MemoryCell])
...
splitAt3 loc1 loc2 cells = 
  let mytail = snd fullTail
      ...
      fullTail = 
        (splitAt (loc2 - loc1)
                 (snd fullHead))
  in (myhead,middle,mytail)---- EXPERIMENTATION RELATED TO MUTABLE VECTORS AND ST MONAD, ETC. 
